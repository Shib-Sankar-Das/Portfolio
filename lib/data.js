import { certificatesByDate, certificatesForSlugs } from "./certificates";
...
/** Certificates listed on a domain route, in that domain's chosen order. */
export function certificatesForDomain(slug) {
  return certificatesForSlugs(getDomain(slug)?.certificateSlugs);
}

/** Every certificate, newest first — used by the primary page. */
export function allCertifications() {
  return certificatesByDate();
}

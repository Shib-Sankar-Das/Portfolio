...
function toCertificate(row) {
  ...
  return {
    ...
    // (PDFs get a rendered first page), then the raw asset, then — for a
    // bundle with no cover of its own — the first document that has one.
    image: row.preview_url || row.asset_url || cover,
    assetUrl: row.asset_url,
    assetPublicId: row.asset_public_id,
    assetFormat: row.asset_format,
    assetKind: row.asset_kind,
